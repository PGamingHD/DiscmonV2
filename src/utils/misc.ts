...
export function calculatePokemonHP(
  ...
  return (
    Math.floor(
      0.01 * (2 * BaseEV + HPiv + Math.floor(0.25 * currentEV)) * Level
    ) +
    Level +
    10
  );
...
export function calculateDamage(
  ...
  const effectiveness: number =
    typeChart[move.type.toLowerCase()][defenderType[0].toLowerCase()] *
    (defenderType[1]
      ? typeChart[move.type.toLowerCase()][defenderType[1].toLowerCase()]
      : 1);
  const typeMultiplier: 1 | 2 | 0.5 =
    effectiveness === 0 ? 0.5 : effectiveness === 2 ? 2 : 1;
  ...
  const damageDone: number = Math.floor(
    baseDamage * typeMultiplier * criticalMultiplier * randomFactor
  );
