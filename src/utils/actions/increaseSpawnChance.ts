import {channelIncense, PokemonServer, TrainerRanks, userData} from "@prisma/client";
import {ExtendedClient} from "../../structures/Client";
import {Message} from "discord.js";
import db from "../database";

export default async function(serverData: PokemonServer, client: ExtendedClient, message: Message<boolean>) {
    const usersData: userData | null = await db.findPokemonTrainer(message.author.id);
    ...
    if (!client.awardCooldowns.has(message.guild.id)) {
        ...
        if (getChannelIncense) {
            if (getChannelIncense.incenseTimeout <= Date.now()) {
                ...
                } else if (usersData.trainerRank === TrainerRanks.MODERATOR || usersData.trainerRank === TrainerRanks.ADMINISTRATOR || usersData.trainerRank === TrainerRanks.DEVELOPER) {
