...
export default new Command({
    ...
    run: async ({ interaction, client }) => {
        ...
        if (findPokemon.pokemonFavorite) {
            ...
            return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.GREEN).setDescription('You have successfully removed this Pokémon from your favorites list.')]});
        } else {
            await db.setPokemonFavorite(findPokemon.pokemonId, true);

            return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.GREEN).setDescription('You have successfully added this Pokémon to your favorites list.')]});
        }
    }
});