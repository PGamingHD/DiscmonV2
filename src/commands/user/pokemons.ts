...
import {PokemonOrder, PokemonRarity, userData} from "@prisma/client";
    ...
    run: async ({ interaction, client }) => {
        ...
        if (rarity) {
            ...
            for (const pokemon of ownedPokemons) {
                if (rarity.toUpperCase() === "SHINY" && !pokemon.pokemonPicture.includes('shiny') || rarity.toUpperCase() !== pokemon.pokemonRarity && rarity.toUpperCase() !== "SHINY") {
                    continue;
                }
            ...
            if (pokemonData.length === 0) return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.RED).setDescription('You do not have any Pokémons to display in this rarity.')]});
            ...
            for (const page of pages) {
                ...
                embeds.push({
                    ...
                    footer: {
                        text: `Page ${currentPage} of ${pages.length} - Sorted by: ${pokemonTrainer.pokemonOrder} | Rarity: ${capitalizeFirst(rarity)}`
                    },
                    color: Colours.MAIN
                })
            }
        ...
        for (const page of pages) {
            ...
            embeds.push({
                ...
                description: `${page.join('\n')}`,
                footer: {
                    text: `Page ${currentPage} of ${pages.length} - ${pokemonTrainer.pokemonOrder === PokemonOrder.LEVEL || pokemonTrainer.pokemonOrder === PokemonOrder.ID || pokemonTrainer.pokemonOrder === PokemonOrder.FAVORITE || pokemonTrainer.pokemonOrder === PokemonOrder.IV ? `Sorted by: ${pokemonTrainer.pokemonOrder}` : `Displaying: ${pokemonTrainer.pokemonOrder}`}`
                },
                color: Colours.MAIN
            })
        }
