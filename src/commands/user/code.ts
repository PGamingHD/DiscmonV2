...
import {globalCodes, Pokemons, RewardType, userCodes, userData} from "@prisma/client";
...
export default new Command({
    ...
    run: async ({ interaction, client }) => {
        ...
        if (interaction.guild.id !== process.env.guildId) return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.RED).setDescription('Codes may only be redeemed on our official Discord Server.')]});

        if (!redeem) {
            return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.MAIN).setTitle('What are codes?').setDescription('Codes are very special that you could redeem and get rewards.\nThese codes are not given out that often and are given in special occations only.\nMake sure to redeem these rewards quick, because they are limited.\n\n*Codes are extemely limited, and can only be obtained through the official discord.*')]});
        } else {
            ...
            return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.GREEN).setDescription(`You have successfully redeemed the code \`${redeem}\` and been awarded with **${toRedeem.rewardAmount.toLocaleString('en-US')}** *${capitalizeFirst(toRedeem.rewardType)}*`)]});
        }
    },
});
