    ...
    run: async ({ interaction, client }) => {
        ...
        collector.on('collect', async (i: ButtonInteraction<CacheType>): Promise<void> => {
            ...
            if (i.customId === "bulbasaur") {
                ...
                await db.setNewPokemonOwner(generateFlake(), interaction.user.id, pokemon.pokemonPicture, pokemon.pokemonName, Nature[Math.random() * Nature.length>>0], Gender[Math.random() * Gender.length>>0], pokemon.pokemonRarity, true, 1, {
                    HP: HPiv,
                    Attack: ATKiv,
                    Defense: DEFiv,
