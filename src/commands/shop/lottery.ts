...
export default new Command({
    ...
    run: async ({ interaction, client }) => {
        ...
        if (!buy) {
            ...
            if (Number(findGlobals.currentlyEnding) - 900000 < Date.now() || (Number(findGlobals.currentlyEnding) - Date.now()) < 900000) return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.RED).setDescription('Ticket purchases are closed **15 minutes** before and after lottery draws.')]});
