...
export default new Command({
    ...
    options: [{
        ...
        name: 'value',
        description: 'Enter amount to give to user, can use abbreviation b, m, k after number',
        type: ApplicationCommandOptionType.String,
        required: true,
    }],
    run: async ({ interaction, client }) => {
        ...
        if (type === "pokecoins") {
            ...
            const targetsData: userData | null = await db.findPokemonTrainer(trainer.id);
            if (!targetsData) return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.RED).setDescription(`The user selected does not have an account yet.`)]});
            if (!usersData) return;
            ...
            return interaction.reply({ephemeral: true, embeds: [new EmbedBuilder().setColor(Colours.RED).setDescription(`Type to give is not valid, how did it get here?`)]});
        }
    },
});
